...
describe('harmony', () => {
  it('uses standard tuning', () => {
    ...
    expect(midiToHz(60)).toBeCloseTo(261.6256);
  });
...
describe('session validation', () => {
  it('accepts default and serialized states', () => {
    expect(isSession(defaultSession())).toBe(true);
    expect(isSession(JSON.parse(JSON.stringify(defaultSession())))).toBe(true);
  });
  ...
  it.each([NaN, Infinity, 0, 301, '90', null])('rejects invalid BPM %s', (value) => {
    const s = { ...defaultSession(), bpm: value };
    expect(isSession(s)).toBe(false);
  });
  it('rejects unsafe IDs, duplicates, fractions and unknown versions', () => {
    const s = defaultSession();
    s.layers[0].id = '<script>';
    expect(isSession(s)).toBe(false);
    s.layers[0].id = s.layers[1].id;
    expect(isSession(s)).toBe(false);
    s.layers[0].id = 'one';
    s.layers[0].beatsPerCycle = 2.5;
    expect(isSession(s)).toBe(false);
    expect(isSession({ ...defaultSession(), version: 2 })).toBe(false);
    expect(isSession({ ...defaultSession(), layers: [] })).toBe(false);
  });
  it('accepts four fixed layers and rejects a fifth', () => {
    const session = defaultSession();
  ...
  it('assigns and validates a persistent color for every layer', () => {
    const session = defaultSession();
    expect(session.layers.map((layer) => layer.color)).toEqual(LAYER_COLORS.slice(0, 4));
...
describe('transport', () => {
  it('preserves phase across tempo changes and pause/resume', () => {
    ...
    clock.configure(11, 60, 4);
    expect(clock.position(11)).toBe(0.5);
    expect(clock.position(13)).toBe(1);
    clock.pause(13);
    expect(clock.position(100)).toBe(1);
    clock.start(100);
    expect(clock.position(102)).toBe(1.5);
    clock.stop();
    expect(clock.position(110)).toBe(0);
  });
  it('supports a future anchor without negative phase', () => {
    const clock = new TransportClock();
    clock.start(1);
    expect(clock.position(0)).toBe(0);
    expect(clock.timeAt(0)).toBe(1);
  });
});
