...
const beatMarkers = (beats: number, radius: number, color: string, layerNumber: number) =>
  Array.from({ length: beats }, (_, beat) => {
    ...
    return `<g><circle data-beat="${beat / beats}" cx="${x}" cy="${y}" r="${beat === 0 ? 9 : 6}" fill="${color}"/>${beat === 0 ? `<text x="${x}" y="${coordinate(Number(y) + 3.5)}" text-anchor="middle" fill="#171b19" class="beat-number">${layerNumber}</text>` : ''}</g>`;
  }).join('');
...
export interface VisualOptions { visualMotion: 'pointer' | 'runners'; }
export function renderVisual(state: SessionState, options: VisualOptions = { visualMotion: 'pointer' }): string {
  ...
  if (state.visualMode === 'timeline') {
    return `<svg viewBox="0 0 480 480" role="img" aria-label="Oś czasu rytmu ${ratio}">
      ...
      ${options.visualMotion === 'pointer' ? '<line id="timeline-head" x1="50" x2="50" y1="105" y2="365" stroke="#ece8df" stroke-width="1.5"/>' : ''}
  ...
  if (state.visualMode === 'polygons') {
    return `<svg viewBox="0 0 480 480" role="img" aria-label="Wielokąty rytmu ${ratio}">
      <defs><radialGradient id="polygon-halo"><stop stop-color="#d3b47b" stop-opacity=".07"/><stop offset="1" stop-color="#d3b47b" stop-opacity="0"/></radialGradient></defs>
      ...
      ${state.layers.map((layer, index) => {
        ...
        return `<g opacity="${opacity}">${shape}${beatMarkers(layer.beatsPerCycle, radius, layer.color, index + 1)}${runner}</g>`;
      }).join('')}
      ${subdivisionMarkers(state.cycleBeats, state.subdivision, 'polygons')}
      ${options.visualMotion === 'pointer' ? '<g id="polygon-head"><line x1="240" y1="163" x2="240" y2="32" stroke="#f3eee3" stroke-opacity=".5"/><path d="M235 39 L240 29 L245 39" fill="none" stroke="#f3eee3"/></g>' : ''}
      <circle cx="240" cy="240" r="3" fill="#f3eee3" opacity=".7"/>
  ...
  return `<svg viewBox="0 0 480 480" role="img" aria-label="Rytm ${ratio}. Pierwsze uderzenie na godzinie dwunastej.">
    <defs><radialGradient id="halo"><stop stop-color="#d3b47b" stop-opacity=".055"/><stop offset="1" stop-color="#d3b47b" stop-opacity="0"/></radialGradient></defs>
    <circle cx="240" cy="240" r="232" fill="url(#halo)"/>
    ${Array.from({ length: 60 }, (_, i) => { const a = i * Math.PI / 30; return `<line x1="${240 + Math.sin(a) * 216}" y1="${240 - Math.cos(a) * 216}" x2="${240 + Math.sin(a) * (i % 5 === 0 ? 223 : 219)}" y2="${240 - Math.cos(a) * (i % 5 === 0 ? 223 : 219)}" stroke="${i % 5 === 0 ? '#68716a' : '#343d36'}"/>`; }).join('')}
    ${state.layers.map((l, i) => { const r = 185 - i * 32; return `<g opacity="${l.muted || (solo && !l.solo) ? .2 : 1}"><circle cx="240" cy="240" r="${r}" fill="none" stroke="${l.color}" stroke-opacity=".27" stroke-width="1"/>
      ${beatMarkers(l.beatsPerCycle, r, l.color, i + 1)}${options.visualMotion === 'runners' ? `<circle class="visual-runner circle-runner" data-radius="${r}" cx="240" cy="${240 - r}" r="5" fill="${l.color}"/>` : ''}</g>`; }).join('')}
    ${subdivisionMarkers(state.cycleBeats, state.subdivision, 'circle')}
    ${options.visualMotion === 'pointer' ? '<g id="circle-head"><line x1="240" y1="172" x2="240" y2="30" stroke="#f3eee3" stroke-opacity=".45"/><circle cx="240" cy="30" r="3" fill="#f3eee3"/></g>' : ''}
    <text x="240" y="230" text-anchor="middle" class="svg-ratio ${state.layers.length > 2 ? 'small' : ''}">${ratio}</text>
