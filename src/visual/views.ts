...
export function renderVisual(
  state: SessionState,
  options: VisualOptions = { visualMotion: 'pointer' },
): string {
  const theme =
    options.theme ??
    createVisualTheme({
    ...
    return `<svg viewBox="0 0 480 480" role="img" aria-label="Wielokąty rytmu ${ratio}">
      ...
      ${layers
        .map((layer, index) => {
          ...
          const runner =
            options.visualMotion === 'runners'
              ? `<circle class="visual-runner polygon-runner" data-sides="${layer.beatsPerCycle}" data-radius="${radius}" cx="240" cy="${240 - radius}" r="5" fill="${layer.color}"/>`
              : '';
      ...
      <text x="240" y="230" text-anchor="middle" class="svg-ratio ${layers.length > 2 ? 'small' : ''}">${ratio}</text>
...
export function animateVisual(
  ...
  head?.setAttribute('x1', String(50 + phase * 384));
  head?.setAttribute('x2', String(50 + phase * 384));
  container
    .querySelectorAll<SVGCircleElement>('.timeline-runner')
    .forEach((dot) => dot.setAttribute('cx', String(50 + phase * 384)));
  container.querySelectorAll<SVGCircleElement>('.circle-runner').forEach((dot) => {
    const radius = Number(dot.dataset.radius),
  ...
  if (count) {
    const language = PREFERENCE_LANGUAGES.includes(document.documentElement.lang as Language)
      ? (document.documentElement.lang as Language)
      : 'en';
    count.textContent = translateText(
      `CYKL ${String(Math.floor(position) + 1).padStart(2, '0')}`,
      language,
    );
  }
