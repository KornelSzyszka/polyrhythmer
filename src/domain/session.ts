...
export function isSession(v: unknown): v is SessionState {
  if (!object(v) || v.version !== 1 || !range(v.bpm, 20, 300) || !range(v.cycleBeats, 1, 16, true) || !range(v.subdivision, 0, 6, true) || !range(v.masterGain, 0, 1) || !['circle', 'timeline', 'polygons'].includes(String(v.visualMode))) return false;
