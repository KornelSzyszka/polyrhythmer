...
export interface PreferencesState { version: 1; language: Language; developerMode: boolean; visualMotion: 'pointer' | 'runners'; activePaletteId: string; palettes: Palette[]; }
...
export const defaultPreferences = (): PreferencesState => ({ version: 1, language: 'en', developerMode: false, visualMotion: 'pointer', activePaletteId: 'forest', palettes: [] });
...
export const isPreferences = (value: unknown): value is PreferencesState => object(value) && value.version === 1 && PREFERENCE_LANGUAGES.includes(value.language as Language) && typeof value.developerMode === 'boolean' && ['pointer', 'runners'].includes(String(value.visualMotion)) && typeof value.activePaletteId === 'string' && Array.isArray(value.palettes) && value.palettes.length <= 20 && value.palettes.every(isCustomPalette) && new Set(value.palettes.map(palette => palette.id)).size === value.palettes.length && !!paletteById({ version: 1, language: value.language as Language, developerMode: value.developerMode, visualMotion: value.visualMotion as PreferencesState['visualMotion'], activePaletteId: value.activePaletteId, palettes: value.palettes }, value.activePaletteId);
